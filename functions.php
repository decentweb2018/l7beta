...
function _l7_setup() {
	...
	register_nav_menus( array(
		...
		'footer-menu'     => esc_html__( 'Footer Menu', '_l7_theme' )
	) );
...
function _l7_scripts() {
	...
	wp_enqueue_style( 'animate-css', get_stylesheet_directory_uri() . '/assets/vendor/animate.css/animate.min.css', array(), ASSETS_VERSION );
	wp_enqueue_script( 'wowjs', get_stylesheet_directory_uri() . '/assets/vendor/wow/dist/wow.js', array('jquery'), ASSETS_VERSION, true );
	wp_enqueue_style( 'slick-css', get_stylesheet_directory_uri() . '/assets/vendor/slick-carousel/slick/slick.css', array(), ASSETS_VERSION );
	wp_enqueue_style( 'slick-css-theme', get_stylesheet_directory_uri() . '/assets/vendor/slick-carousel/slick/slick-theme.css', array(), ASSETS_VERSION );
	wp_enqueue_script( 'slick', get_stylesheet_directory_uri() . '/assets/vendor/slick-carousel/slick/slick.min.js', array('jquery'), ASSETS_VERSION, true );
	wp_enqueue_script( 'autogrow', get_stylesheet_directory_uri() . '/assets/vendor/jquery.autogrow-textarea/jquery.autogrow-textarea.js', array('jquery'), ASSETS_VERSION, true );
	wp_enqueue_script( 'jquery-cookie', get_stylesheet_directory_uri() . '/assets/vendor/jquery.cookie/jquery.cookie.js', array( 'jquery' ), ASSETS_VERSION, true );


	if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
		wp_enqueue_style( 'l7_icomoon', 'https://i.icomoon.io/public/3eb0997007/L7/style.css', array(), ASSETS_VERSION );
		wp_enqueue_style( '_l7_theme-style', get_stylesheet_directory_uri() . '/assets/build/styles.css', array('l7_icomoon'), ASSETS_VERSION );
...
function post_qa_body($atts) {
	...
	$the_qa_output .= '<div class="qa_question" style="color:'.$themecolor.'">';
...
// Add support for L7 autocomplete.
wp_enqueue_script('jqueryui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js', array('jquery'));
wp_enqueue_script('mysite-js', get_stylesheet_directory_uri().'/assets/js/mysite.js', array('jqueryui'));
wp_enqueue_style('autocomplete.css', 'https://code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css');

// Register autocomplete server side function.
add_action('wp_ajax_nopriv_l7_autocomplete', 'l7_autocomplete');
add_action('wp_ajax_l7_autocomplete', 'l7_autocomplete');

function l7_autocomplete() {
	global $wpdb;

	$term = '%'.$wpdb->esc_like(stripslashes($_REQUEST['term'])).'%';

	$from_clause_array = [];
	$terms = [];
	foreach (explode(',', $_REQUEST['fields']) as $value) {
		array_push($from_clause_array, preg_replace('/[^A-Za-z_]/', "", $value).' like %s');
		array_push($terms, $term);
	}

	$sql = "select id,post_title
		from $wpdb->posts
		where (" . implode(' OR ', $from_clause_array) . ")
		and post_status = 'publish' and (post_type = 'post' OR post_type = 'page')";


	$sql = $wpdb->prepare($sql, $terms);

	$results = $wpdb->get_results($sql);

	$titles = array();
	foreach($results as $r) {
		$titles[] = array('label' => $r->post_title, 'value' => $r->id);
	}
...
require get_template_directory() . '/inc/sidemenu-walker.php';
